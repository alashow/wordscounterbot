...
def parseCommandText(body):
	...
	if match and match.group(1):
		words = match.group(6)
		words = config.DEFAULT_TARGET_WORDS if (words is None) else words.split(',')
		words = config.N_WORDS if "nword" in match.group(1) else words

		return (match.group(2), match.group(4), words)
	else:
		return False
...
def processComment(comment):
	...
	if result:
		(bot, user, words) = result
		thread = threading.Thread(target=processSummoning, args=[comment, user, words])
		thread.start()

def processSummoning(comment, user, words):
	...
	if user:
		...
		count, countNR = analyzeUser(user, words, comment)
		replyToComment(comment, user, words, count, countNR)
	...
	if result:
		...
		if user:
			if isUserBlacklisted(user):
				print(f"Skipping blacklisted user {user}")
				return
			count, countNR = analyzeUser(user, words)
			replyToMessage(message, user, words, count, countNR)
		else:
			logging.debug("Message didn't have a target body, skipping.")

def analyzeUser(user, words=config.N_WORDS, comment = None):
	...
	comments = list(config.api.search_comments(author=user, filter=['body'], before=int(recentComments[-1].created_utc), q="|".join(words), size=1000))

	totalMatches = 0
	totalNRMatches = 0
	for s in submissions:
		totalMatches += countTextForWords(words, s.title) + countTextForWords(words, s.selftext) if(hasattr(s, 'selftext')) else 0
		if isNwords:
	...
	for c in (recentComments+comments):
		totalMatches += countTextForWords(words, c.body)
		if isNwords:
...
def replyToComment(comment, user, words, count, countNR):
...
def replyToMessage(message, user, words, count, countNR):
	replyText = utils.buildCounterReply(user, words, count, countNR);
